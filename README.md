...
```bash
Open any browser and type http://localhost:3500. You can now access the Grafana Web UI using the default login/password admin/admin
```

```
docker buildx build --platform linux/amd64 . -t k3s-pyuniversity 
docker tag k3s-pyuniversity:latest docker.mghcloud.com/flask/k3s-pyuniversity:latest
docker push docker.mghcloud.com/flask/k3s-pyuniversity:latest
docker run -p 3500:3500 --restart always --name k3s-pyuniversity-local k3s-pyuniversity:latest
docker logs -f k3s-pyuniversity-local
docker stop k3s-pyuniversity-local
docker rm k3s-pyuniversity-local
